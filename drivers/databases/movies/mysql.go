...
import (
	"context"
	"fmt"
	_genre_b "project/business/genres"
	"project/business/movies"

	// "project/drivers/databases/ratings"
...
func (rep *MysqlMovieRepository) SearchMovie(ctx context.Context, title string) ([]movies.Movie, error) {
	...
	result := rep.Connect.Preload("Genre").Where("title LIKE ?", title+"%").Find(&movie)
	if result.Error != nil {
		return []movies.Movie{}, result.Error
	}
...
func (rep *MysqlMovieRepository) DeleteAll(ctx context.Context) error {
	...
	result = rep.Connect.Delete(&movie)
	if result.Error != nil {
		return result.Error
	}
...
func (rep *MysqlMovieRepository) DeleteMovie(ctx context.Context, id int) error {
	var movie Movies
